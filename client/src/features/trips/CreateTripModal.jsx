...
export const CreateTripModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl bg-[#F7F4EE] border border-[#CBD5D6] rounded p-8 sm:p-12 shadow-2xl overflow-y-auto max-h-[90vh]">
        {/* Close button */}
        <button
          type="button"
          onClick={onClose}
          className="absolute top-6 right-6 text-[#899596] hover:text-[#202525] transition-colors p-2"
        ...
        <div className="mb-8 space-y-2">
          <span className="text-xs font-semibold uppercase tracking-[0.2em] text-[#163A3D]">
            NEW JOURNEY BUILDER
          </span>
          <h2 className="font-serif text-3xl sm:text-4xl font-bold text-[#202525]">
            Create your journey
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="relative">
            ...
            {autocompleteResults.length > 0 && (
              <div className="absolute top-full left-0 right-0 z-30 mt-1 bg-[#FFFFFF] border border-[#CBD5D6] rounded shadow-lg max-h-48 overflow-y-auto">
                {autocompleteResults.map((item) => (
                  <button
                    ...
                    className="w-full text-left px-4 py-3 text-xs hover:bg-[#F6F3F2] flex items-center justify-between border-b border-[#EDE7DF] last:border-0"
                  >
                    <span className="font-semibold text-[#202525]">
                      {item.name}
                    </span>
                    <span className="text-[#899596]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <input
                ...
                className="w-full bg-transparent border-0 border-b border-[#CBD5D6] py-3 text-base text-[#202525] focus:ring-0 focus:border-[#163A3D] transition-colors"
              />
            </div>
          ...
          <div className="pt-4 flex items-center justify-end gap-4">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-3 border border-[#CBD5D6] text-[#54433A] rounded text-xs font-semibold uppercase tracking-wider hover:bg-[#F6F3F2] transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="bg-[#163A3D] text-white px-8 py-3.5 rounded text-xs font-semibold uppercase tracking-widest hover:bg-[#204F53] transition-colors shadow-sm inline-flex items-center gap-2 cursor-pointer disabled:opacity-50"
            >
