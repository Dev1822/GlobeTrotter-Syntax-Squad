  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-[#FFFFFF] p-6 sm:p-8 border border-[#E5E2E1] rounded shadow-xs space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2">
            <span className="font-serif text-2xl font-bold text-[#163A3D]">
              {packedCount}/{totalCount}
            </span>
            <span className="text-xs text-[#899596]">
        ...
        {/* Visual Progress Bar */}
        <div className="w-full bg-[#EDE7DF] h-2 rounded-full overflow-hidden">
          <div
            className="h-full bg-[#163A3D] transition-all duration-300 ease-out"
            style={{ width: `${progressPercent}%` }}
          />
        </div>

        {/* Template Loaders */}
        <div className="pt-4 border-t border-[#EDE7DF] flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-[#899596] flex items-center">
              <Sparkles className="w-3.5 h-3.5 mr-1 text-[#163A3D]" />
              Load Template:
            </span>
            <button
              type="button"
              onClick={() => handleApplyTemplate("beach")}
              className="px-2.5 py-1 bg-[#F6F3F2] hover:bg-[#FFDBC9] text-[#163A3D] font-semibold rounded transition-colors"
            >
              Beach
            </button>
            <button
      ...
      <form
        ...
        className="p-4 sm:p-6 bg-[#FFFFFF] border border-[#CBD5D6] rounded-md shadow-xs flex flex-col sm:flex-row items-center gap-4"
      >
        <div className="flex-1 w-full">
          <input
            ...
            className="w-full py-2 bg-transparent border-b border-[#CBD5D6] text-sm text-[#202525] placeholder-[#899596]/70 focus:outline-none focus:border-[#163A3D]"
          />
          {addError && (
        ...
        <div className="w-full sm:w-40">
          <select
            ...
            className="w-full py-2 bg-transparent border-b border-[#CBD5D6] text-xs font-semibold text-[#54433A] focus:outline-none focus:border-[#163A3D]"
          >
            {categories.map((cat) => (
          ...
          {filteredItems.map((item) => (
            ...
            >
              <div className="flex items-center space-x-4">
                <button
                  type="button"
                  className="text-[#163A3D] focus:outline-none"
                  aria-label={
                ...
                >
                  {item.packed ? (
                    ...
                    <Square className="w-5 h-5 text-[#CBD5D6] hover:text-[#163A3D]" />
                  )}
                </button>
                ...
                <div>
                  <span
                    className={`text-sm font-medium transition-all ${
                      item.packed
                        ? "line-through text-[#899596]"
                        : "text-[#202525]"
                    }`}
                  >
                    {item.name}
                  </span>
                  <div className="text-[10px] text-[#899596] font-semibold uppercase tracking-wider mt-0.5">
                    {item.category}
                  </div>
                </div>
              </div>
